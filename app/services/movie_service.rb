...
class MovieService
  ...
  def movie_cast(movie_id)
    get_url("/movie/#{movie_id}/credits?api_key=#{ENV['movie_api_key']}&language=en-US")[:cast]
  end

  def movie_review(movie_id)
    get_url("/movie/#{movie_id}/reviews?api_key=#{ENV['movie_api_key']}&language=en-US")[:results]
  end
